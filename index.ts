const HEIGHT_STAFF_BRAVURA = 1000
...
const drawNote = (ctx: CanvasRenderingContext2D, topOfStaff: number, left: number, index: number, scale: number) => {
  ...
  const e2y = topOfStaff + 1750 * scale // scale=1, index=0(E2)のときY=1750
  const top = e2y - (index * halfOfNoteHeadHeight) + halfOfNoteHeadHeight
...
window.onload = () => {
  ...
  drawNote(canvasCtx, topOfStaff, 250, 21, scale)
}