...
const drawNote = (ctx: CanvasRenderingContext2D, topOfStaff: number, left: number, index: number, scale: number) => {
...
const drawLedgerLines = (ctx: CanvasRenderingContext2D, topOfStaff: number, leftOfNoteHead: number, indexOfNote: number, scale: number) => {
...
const drawStem = (ctx: CanvasRenderingContext2D, topOfStaff: number, leftOfNoteHead: number, indexOfNote: number, scale: number) => {
  ...
  if (indexOfNote < 12) {
    // B3より低い -> 上向き
    left = leftOfNoteHead + WIDTH_NOTE_HEAD * scale
    bottom = indexToY(topOfStaff, indexOfNote, scale)
    if (indexOfNote < 5) {
      // C3より低い -> topはB3
      top = heightOfB3
    } else {
  ...
  ctx.strokeStyle = "#000"
  ctx.lineWidth = WIDTH_STEM * scale
  ctx.beginPath()
  ctx.moveTo(left, top)
